    ...
    stages {
        ...
        stage('Security') {
            steps {
                ...
                bat "docker run --rm ${DOCKER_IMAGE}:${DOCKER_TAG} python -m bandit -r app/ -f json -o bandit-report.json || true"
                bat "docker run --rm ${DOCKER_IMAGE}:${DOCKER_TAG} python -m bandit -r app/ || true"
        ...
        stage('Deploy') {
            steps {
                ...
                bat "docker stop ${CONTAINER_NAME}-staging || true"
                bat "docker rm ${CONTAINER_NAME}-staging || true"
        ...
        stage('Release') {
            steps {
                ...
                bat "docker stop ${CONTAINER_NAME}-prod || true"
                bat "docker rm ${CONTAINER_NAME}-prod || true"
        ...
        stage('Monitoring') {
            steps {
                ...
                bat "docker stop prometheus || true"
                bat "docker rm prometheus || true"
                bat "docker stop grafana || true"
                bat "docker rm grafana || true"
